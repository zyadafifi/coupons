...
- Checks custom claims or UID allowlist from environment variable
...
2. Edge Functions token verification (push notifications)
...
- FCM tokens: Only length logged in production
...
if (import.meta.env.DEV) {
  ...
  console.log('[Push] FCM Token (length:', token.value.length, ')');
}
```
...
3. Set secrets:
   ```bash
   # Firebase Service Account (full JSON)
   supabase secrets set FIREBASE_SERVICE_ACCOUNT='{"type":"service_account",...}'
   
   # Admin UID Allowlist (comma-separated, optional if using custom claims)
   supabase secrets set ADMIN_UID_ALLOWLIST='uid1,uid2,uid3'
   ```

4. Redeploy functions:
   ```bash
   supabase functions deploy send-push-notification
   supabase functions deploy get-admin-notifications
   ```
...
- Check Supabase logs for auth success
