        ...
        <div className="relative px-3 py-3 pb-5 bg-primary">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 text-primary-foreground min-w-0">
              <p className="text-sm font-medium opacity-90 truncate">{storeName}</p>
              <p className="font-mono font-bold text-lg tracking-wider truncate">{code || discount}</p>
            </div>

            {/* Discount Badge */}
            {code && discount && (
              <div className="shrink-0 bg-card/20 backdrop-blur-sm px-2 py-1 rounded-lg">
                <span className="text-primary-foreground font-bold text-sm">{discount}</span>
              </div>
            )}
          </div>
        </div>
