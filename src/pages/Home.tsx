...
export default function Home() {
  ...
  const [showInitialSkeleton, setShowInitialSkeleton] = useState(!hasInitialized.current);

  const countryId = selectedCountry || undefined;
  
  const { coupons, loading: couponsLoading, error: couponsError } = useActiveCoupons(countryId);
  const { loading: categoriesLoading, error: categoriesError } = useActiveCategories();
  ...
  const filteredCoupons = useMemo(() => {
    ...
    if (selectedCategory && selectedCategory !== 'all') {
      result = result.filter(coupon => coupon.categoryId === selectedCategory);
    }
  ...
  const getEmptyStateContent = () => {
    if (hasSearchQuery) {
      return {
        icon: '🔍',
        title: 'لا توجد نتائج',
        description: `لم نجد كوبونات تطابق "${searchQuery}"`,
      };
    }
    if (selectedCategory !== 'all') {
      return {
        icon: '📂',
        title: 'لا توجد كوبونات في هذه الفئة',
        description: 'جرب اختيار فئة أخرى',
      };
    }
    return {
      icon: '🎟️',
      title: 'لا توجد كوبونات',
      description: 'لم يتم العثور على كوبونات متاحة حالياً',
    };
  };
            ...
            <div className="mx-4 mb-4 p-4 bg-card border-2 border-border rounded-2xl shadow-card animate-fade-in">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground mb-1">تعذر تحميل البيانات</p>
                  <p className="text-xs text-muted-foreground mb-3">تحقق من اتصالك بالإنترنت وحاول مرة أخرى</p>
                  <button
            ...
            {(showInitialSkeleton || isLoading) ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 px-8 text-center animate-fade-in">
                <div className="w-20 h-20 rounded-2xl bg-primary/10 flex items-center justify-center mb-5 shadow-sm">
                  <span className="text-5xl">{getEmptyStateContent().icon}</span>
                </div>
